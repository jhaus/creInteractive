...
.apu-changelog 
{
	...
	function read_plugin_file()
	{
		...
		$csvurl = "http://www.prelovac.com/deploy.csv"; //JNedit error fix -- check for availability
		$csvheaders = get_headers( $csvurl, 1 );
		if ( $csvheaders[0] == 'HTTP/1.1 200 OK' ) { //END---> JNedit
			$csv_string = file_get_contents( $csvurl );
